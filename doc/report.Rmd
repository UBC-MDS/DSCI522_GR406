...
model_quality <- readRDS("../results/summary_table.rds")
```
...
__Key predictor__: This is our variable of interest, the `time of day` group variable as well as `day of week`. We want to know how the Outcome variable changes with this key predictor. <br>
...
tidy(model_quality) %>% select(term, estimate, p.value) %>% 
  kable(caption = "Table 1. Summary Table of Model Parameters.", format = 'html') %>% 
  kable_styling() %>%
  row_spec(10:16, bold = T, color = "black", background = "#B0C4DE")
```
